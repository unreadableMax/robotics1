...
Vector3d forwardKinematic(Vector6d q) {
	...
	//Ein linkshändiges Koordinatensystem wird verwendet

	//Translations-Vektoren x, y, z [m] (vgl. Fig 2 und 3)
	...
	Vector3d usedYAxis(0.0,-1.0,0.0); //-1 weil linkshändig
	Vector3d usedZAxis(0.0,0.0,1.0);

	Vector3d strangeVector(2.0,0.0,0.0);
		
	//Forwärtskinematik-Formel anwenden. Wichtig: Globale interpretation, linkshändig
	Vector3d TCP = t6;
	...
	TCP = t0 + (AngleAxisd(-q[0], usedZAxis) * TCP).cross(strangeVector);
	return TCP;
